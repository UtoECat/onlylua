...
#define LUA_LIB// include lprefix.h 
...
#if defined(LUA_USE_APICHECK)// include assert.h 
...
#if defined LUAI_ASSERT// include assert.h 
...
#define l_inspectstat(stat,what)     if (WIFEXITED(stat)) { stat = WEXITSTATUS(stat); }    else if (WIFSIGNALED(stat)) { stat = WTERMSIG(stat); what = "signal"; }
...
#if !defined(luai_makeseed)// include time.h 
...
#if !defined(l_randomizePivot)		// include time.h 
...
static void freeblock (Memcontrol *mc, Header *block) {
  if (block) {
    ...
    fillmem(block, sizeof(Header) + size + MARKSIZE);  
    free(block);  
    mc->numblocks--;  
    mc->total -= size;
  }
...
void *debug_realloc (void *ud, void *b, size_t oldsize, size_t size) {
  ...
  if (size > oldsize && mc->total+size-oldsize > mc->memlimit)
    ...
    newblock = cast(Header *, malloc(realsize));  
    if (newblock == NULL)
      return NULL;  
    if (block) {
      memcpy(newblock + 1, block + 1, commonsize);  
      freeblock(mc, block);  
    }
...
static int loadlib (lua_State *L) {
  ...
  luaL_requiref(L1, "package", NULL, 1);  
...
static int panicback (lua_State *L) {
  struct Aux *b;
  lua_checkstack(L, 1);  
  lua_getfield(L, LUA_REGISTRYINDEX, "_jmpbuf");  
...
static int checkpanic (lua_State *L) {
  ...
  lua_atpanic(L1, panicback);  
  lua_pushlightuserdata(L1, &b);
  lua_setfield(L1, LUA_REGISTRYINDEX, "_jmpbuf");  
  if (setjmp(b.jb) == 0) {  
    runC(L, L1, code);  
...
static void Chook (lua_State *L, lua_Debug *ar) {
  ...
  lua_gettable(L, -2);  
  scpt = lua_tostring(L, -1);  
  lua_pop(L, 2);  
  lua_pushstring(L, events[ar->event]);  
  lua_pushinteger(L, ar->currentline);  
  runC(L, L, scpt);  
}
...
static void sethookaux (lua_State *L, int mask, int count, const char *scpt) {
  ...
  if (!lua_istable(L, -1)) {  
    ...
    lua_newtable(L);  
    lua_pushvalue(L, -1);
    lua_setfield(L, LUA_REGISTRYINDEX, "C_HOOK");  
  }
  lua_pushlightuserdata(L, L);
